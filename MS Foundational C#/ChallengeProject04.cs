...
{
    internal class ChallengeProject04
    {
...
*/
            ...
            while (transactions > 0)
            {
                ...
                int paymentOnes = (itemCost % 2 == 0) ? valueGenerator.Next(0, 3) : valueGenerator.Next(1, 3); // More likely to use 1s
                int paymentFives = (itemCost % 10 > 7) ? valueGenerator.Next(0, 2) : valueGenerator.Next(0, 3); ; // value is 1 when itemCost ends with 8 or 9, otherwise value is 0
            ...
            {
                int[] previousCashTill = (int[])cashTill.Clone();
                ...
                if (changeNeeded < 0)
                {
                    Array.Copy(previousCashTill, cashTill, previousCashTill.Length);
                ...
                {
                    Array.Copy(previousCashTill, cashTill, previousCashTill.Length);
                    throw new InvalidOperationException("InvalidOperationException: The till can't make change.");
                }
            }
